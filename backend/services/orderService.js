...
const executeBuyOrder = async (order, executionPrice, companyName) => {
  // Use MongoDB session/transaction for atomic execution
  ...
  try {
    ...
    // Find or create portfolio with session
    let holding = await Portfolio.findOne({ userId: user._id, symbol: order.symbol }).session(session);
    ...
    // Update order status inside the same transaction as the financial writes.
...
const executeSellOrder = async (order, executionPrice, companyName) => {
  ...
  try {
    // Fetch user and holding with session. MongoDB sessions cannot run
    // parallel operations inside the same transaction.
    const user = await User.findById(order.userId).session(session);
    const holding = await Portfolio.findOne({ userId: order.userId, symbol: order.symbol }).session(session);
