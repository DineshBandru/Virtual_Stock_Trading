...
const Register = ({ inline = false }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      // Prefer detailed validation errors from server (express-validator)
      const data = err?.response?.data;
      let msg = "Registration failed";
      if (data) {
        if (Array.isArray(data.errors) && data.errors.length > 0) {
          msg = data.errors.map((e) => `${e.path}: ${e.msg}`).join("; ");
        ...
        {error ? (
          <div className="rounded-lg border border-red-500 bg-red-900/60 p-3 text-sm text-red-100">
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1">Registration failed: {error}</div>
              <button
                type="button"
                onClick={() => setError("")}
